...
- **`slowdown_gpio = 3`** — paired with `rp1_rio=1`. Raise to 4–5 if flicker persists (typical for chained panels with fast refresh).

Default bigsign config uses `slowdown_gpio = 3` + `pwm_bits = 8` + `rp1_rio = 1`.
