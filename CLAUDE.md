...
**Visibility scheduling** (`schedule = {...}` on widgets/sections) — `schedule` is a HARD-RESERVED core field: popped (and the reservation check run) in `app/factories.validate_widget_cfg` BEFORE `_run_validate_config` invokes a widget/plugin class's own `validate_config` hook — a strict "reject unknown keys" hook must never see the core-reserved key (never reaches a widget constructor either; a widget class declaring its own `schedule` field is rejected at config-load), parsed STRICTLY by `schedule.parse_visibility_schedule`, and bound to the built widget via the weakref registry in `schedule.py` (`bind_schedule`/`schedule_for` — widgets are slotted+unhashable, hence id-keyed weakrefs). Section titles do NOT accept `schedule` — `_build_title` raises a `ValueError` pointing at scheduling the section instead: titles route through `_build_widget` (so a schedule would validate + bind) but bypass `_expand_sources`, the engine's only gate, so a bound title schedule would silently do nothing. Engine gates: `ticker._schedule_active` in `_expand_sources` (ANDs with `should_display`; a raising check KEEPS the widget) and `run._section_schedule_active` in the section loop. **`_expand_sources`'s gate re-runs every pass through the section, on a bounded notification queue (`maxsize=2`, #394)** — including in a `loop_count = 0` (forever) section, so a widget-level schedule closing (or `should_display()` flipping) reaches the panel within about three in-flight items plus the remainder of the already-expanded pass — up to roughly one full pass for sections with many widgets or a large container — regardless of `loop_count`. A SECTION-level `schedule` doesn't get this: `run._section_schedule_active` checks it once, at section entry, and a `loop_count = 0` section has no natural re-entry point to re-check at (unless the widget rotation itself empties out — e.g. every widget also carries its own gapped `schedule` — handing the forever cycle an exit). `led-ticker validate` warns on that section-level shape; it does NOT warn on widget-level-only schedules in a forever section, since those are already gated at display time. **All-scheduled-out → dark, on BOTH routes**: (1) section-level — the section's own `schedule` is inactive; (2) widget-level — no section-level schedule, but every widget in the section is individually scheduled out (or a Container is currently empty), so `_expand_sources` yields nothing and there's no title either. `run._section_has_content(title, widgets, breaker)` makes this decision ONCE per section pass (reusing its `expanded` return for `first_widget` — no second `_expand_sources` call) and gates whether `_any_section_ran` is set; either route leaves it `False` for a fully-out section, and the outer loop's `_idle_when_all_scheduled_out` debounces before committing: `dark_streak` counts consecutive all-out cycles, and the FIRST one is a no-op (no fetch, no blank, no log — just a 1s sleep) — a FLICKER guard, not a leak guard: a config whose only section flaps content on/off between polls (e.g. a Container emptying on a failed poll) keeps its last frame for one extra second instead of visibly blanking every poll. Dark idle rides `LedFrame`'s recycling; the debounce remains as the flicker guard — the leak guard that used to be a dark-path-local `dark_canvas` retention is now just the frame-level recycling contract (see **Idle keepalive & framebuffer recycling** above): `_blank_swap` Clears + `swap()`s the recycled buffer every ~1s dark iteration (capturing the return, constraint #1), so overlay hooks (e.g. busy_light) keep compositing and the status board's `swap_count` liveness counter keeps advancing all night; it still logs only on the dark/wake transitions, never per iteration (nor during the debounce cycle). A section must never be left showing its last drawn frame while the outer loop busy-spins. `_blank_swap` is also called directly from `run()`'s empty-playlist branch (`_idle_on_empty_playlist` returning `_idled=True`) — unconditionally, dark or not (#396): a hot-reload can land a zero-section config while the panel is already dark, or the playlist can become empty before any dark commit ever happens, and either way the branch keeps blanking instead of stalling swaps. That branch also resets `last_widget`/`last_scroll_pos`/`last_bg_color` every idled iteration before its `continue`, same rationale as `_on_display_dark_transition` below: without it, a reload that restores sections after an empty-playlist interlude would replay the pre-interlude widget as the recovery cycle's entry-transition outgoing frame from a black panel. The outer-top reload branch (mirroring the per-section reload's pin) also sets `_any_section_ran = True` / `_dark_streak = 0` when it lands, so restoring sections after an empty-playlist interlude can't commit a spurious dark on hours-stale streak evidence. `_entry_transition_active` additionally requires `_schedule_active(last_widget)` (alongside `_displayable`) so a widget that went schedule-inactive between sections can't flash as an entry transition's outgoing frame. **The False->True (panel just went dark) transition also resets `last_widget`/`last_scroll_pos`/`last_bg_color`** via `run._on_display_dark_transition` — without it, the morning wake's entry transition would draw yesterday evening's `last_widget` at full brightness as the outgoing frame. `_entry_transition_active`'s `last_widget is not None` guard turns that reset into "no entry transition on wake" (same as boot); `last_scale`/`last_content_height` are wrapper geometry and are deliberately left alone. `TimeWindow` in `schedule.py` is the ONE wrap-semantics implementation shared with brightness windows — never fork it. Clock: `[display] timezone` via `set_schedule_timezone`, refreshed in `_respawn_schedule` (boot + hot-reload) — `timezone` is listed in `reload.RELOADABLE_DISPLAY_FIELDS` (hot-reloadable, no restart). Tripwires: `tests/test_visibility_schedule.py`, `TestScheduleGate` (`tests/test_ticker_expand_sources.py`), `tests/test_widget_schedule_dispatch.py`, `tests/test_run_section_schedule.py`, `tests/test_entry_transition_gate.py`. Docs: <https://docs.ledticker.dev/concepts/scheduling/>.
...
- Examples: `config/config.example.toml` (smallsign), `config/config.bigsign.example.toml` (bigsign with `pixel_mapper_config`, scaling, RP1 tuning), `config/config.firebird.example.toml` (realistic bigsign storefront template — Firebird Yoga), `config/config.halal-cart.example.toml` (maximalist flair/animation showcase — fisheye marquee, `flair.lottery` balls, fireworks transitions, rainbow + lightbulb-marquee borders, shimmer/color_cycle; needs `led-ticker-flair`)
