...
**Pre-existing limitation — single-row image text, hires emoji, `text_scale = 1`**: The single-row path in `_BaseImageWidget._play_with_text` only wraps the canvas in a ScaledCanvas when `text_scale > 1`. With `text_scale = 1` (the default), `pixel_emoji.draw_with_emoji`'s `isinstance(canvas, ScaledCanvas)` gate is False and the emoji falls back to lores 8×8 even on bigsign. Configs that opt into BDF block-expansion via `text_scale = 4` get hires emoji as a side effect; HiresFont configs typically use `text_scale = 1` and end up with lores emoji. Two-row mode (above) doesn't have this issue — it always wraps. Not yet fixed for single-row to avoid changing the rendering of established configs; if/when fixed, mirror the two-row pattern (wrap when `_logical_scale > 1` regardless of `text_scale`).
