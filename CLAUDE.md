...
src/led_ticker/
  ...
  widgets/
    ...
    _gif_decode.py      # Pillow-based decoder: pillarbox/letterbox/stretch/crop fits
    crypto/
...
**GIF widget (`type = "gif"`)**: `GifPlayer` decodes animated GIFs via Pillow at construction time and paints frames directly to the real canvas (bypassing ScaledCanvas — see "Native-resolution painting" above). Four `fit` modes (`pillarbox`, `letterbox`, `stretch`, `crop`) handle aspect-ratio mismatches. `gif_align = "left" | "center" | "right"` anchors pillarboxed gifs horizontally. Transparent GIFs decode through alpha-aware compositing onto a black canvas so the existing skip-black scroll path Just Works. Optional `text` renders alongside the gif: `text_align = "left" | "right"` for static placement (gif under, text over), `"scroll"` for marquee under the gif (skip-black makes text show through transparent / pillar areas), or `"scroll_over"` for marquee always on top. `text_scale > 1` block-expands glyphs via a temporary ScaledCanvas wrapper for text painting only — gif keeps native res. `text_loops = N` puts a floor on marquee passes before the section transitions. Inline `:slug:` emoji work in gif text. Two run modes: `mode = "gif"` is the legacy "panel takeover" path (no titles); `mode = "swap"` is the unified path where gifs work like any other widget alongside an optional title (see `_has_play` dispatch below).
...
10. **`play()`-style widgets must rebind their text/secondary canvases after every swap**: A widget that owns its swap loop (e.g. `GifPlayer.play()`) typically holds two canvas references: one for the gif (real canvas, native pixels) and one for text (a temporary ScaledCanvas wrapper or the same real canvas at scale=1). After `canvas = frame.matrix.SwapOnVSync(canvas)`, the secondary reference is now stale — pointing at the old front buffer that's currently displaying. ScaledCanvas wrappers re-anchor via `wrapper.real = canvas`; raw-canvas references must be reassigned (`text_canvas = canvas`). Skip this rebind and you paint to the displayed buffer every other tick — visible as a "pulsing" flicker on the panel. Tripwire: `test_play_text_scale_1_text_canvas_follows_back_buffer` in `tests/test_widgets/test_gif.py`.
