...
**Single-row image text on bigsign — wrap fires on `_logical_scale`**: `_play_with_text` computes `effective_scale = max(text_scale, _logical_scale)` and wraps when > 1. Three motivating cases: (1) HiresFont + `text_scale=1` on bigsign gets hires emoji because `pixel_emoji.draw_with_emoji`'s `isinstance(c, ScaledCanvas)` gate fires. (2) BDF text + `text_scale=1` on bigsign block-expands to fill the panel instead of rendering at native 12 px. (3) `text_y_offset` is documented as logical pixels and now actually operates on logical pixels everywhere. User intent on `text_scale > 1` is honored as-is (so `text_scale=2` on bigsign still gives 2× block-expansion, not 4×). Tripwires: `TestSingleRowLogicalScaleWrap` (3 tests covering bigsign-wraps, smallsign-no-wrap, user-text-scale-honored). **Mental model**: image widgets follow the same rule as every other widget on the bigsign — content draws at logical scale and the wrapper expands to panel scale. They were the outlier before bd61140; the fix brought them into line with `TickerMessage`, `TwoRowMessage`, and the rest. **Migration note**: existing image-widget configs that relied on `text_scale = 1` rendering small native-12-px BDF text in a pillarbox will now render text at panel-scale and may overlap the image. Two ways to get small text deliberately: (a) HiresFont with explicit `font_size = 12` (panel-aware, real-px sizing — preferred); (b) keep BDF and shrink the image side to leave room (e.g. via `image_align` + smaller text strings). The in-tree `config.gif_text.example.toml` and `config.gif_test.example.toml` PIKACHU sections demo pattern (a) with `font = "Inter-Bold"`, `font_size = 24`.
