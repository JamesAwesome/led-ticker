...
12. **`play()`-style widgets must call `advance_frame()` per tick**: Parallel constraint to #10. The shared engine in `_swap_and_scroll` calls `_advance_frame_if_supported(widget)` per tick so ColorProviders (Rainbow, ColorCycle) animate across hold/scroll. Widgets that own their own render loop (`GifPlayer.play()`, `StillImage.play()` — both routed through `_BaseImageWidget._play_with_text` / `_play_with_two_row_text`) must do the same: `self.advance_frame()` before each per-tick `_render_tick`. Skip it and the provider sees `_frame_count` stuck at the visit-initial value — Rainbow renders as a frozen gradient, ColorCycle freezes on its starting hue. The static-text fast path also gates on the provider's `frame_invariant` flag (both row colors in two-row mode) so non-frame-invariant providers (Rainbow, ColorCycle) are forced through the per-tick loop even when image + geometry are static. `_ConstantColor`, `Random`, and `Gradient` are `frame_invariant=True` and take the fast path; new providers default to `False` (conservative — opt in by setting the class attribute). Tripwires: `TestPlayLoopAdvancesFrame` in `tests/test_widgets/test_image_base.py` (single-row + two-row + fast-path bypass + fast-path retained for constants + Gradient fast-path eligibility).
