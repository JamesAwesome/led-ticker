...
- `make docs-check-llms` — build + verify the agent-facing Markdown export (`/llms.txt`, `/llms-full.txt`, `/llms-small.txt`), generated by `starlight-llms-txt`. Adding or renaming a component (`OptionsTable`, `TomlExample`) that breaks the export fails CI, so run this locally after any component change.
