...
- **`--list-fields` hints are per-widget for plugin fields.** Core's built-in hint table is keyed by bare field NAME, so it carries hints only for fields that core widgets own. A plugin field whose meaning is plugin-specific — most notably `layout`, whose enum differs per plugin (`ticker|two_row` for pool, `auto|strip|big|long` for a forecast, etc.) — gets **no** core hint (a single name-keyed hint would be wrong for all plugins but one; issue #438). Supply your own via a `_LIST_FIELD_HINTS: dict[str, tuple[str, str, str]]` class attribute (`(display_type, description, default_display)`), resolved ahead of the core table. Fields with no per-widget hint fall back to the plain annotation + default. A malformed entry fails `--list-fields` loudly, naming the widget and field. See the [API reference](https://docs.ledticker.dev/plugins/api-reference/) conventions list for an example.
