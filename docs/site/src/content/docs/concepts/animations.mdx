...
Typewriter has `restart_on_visit = true` — the reveal counter resets every time the section shows the widget. With `loop_count = 2`, the widget retypes from scratch on each loop rather than resuming mid-word. With `loop_count = 1` and a multi-widget section in `swap` mode, the widget retypes fresh each time it appears in the rotation. This is the intentional, expected behavior — typewriter is designed to be a reveal, not a persistent animation state. See [frame counters](/concepts/frame-counters/) for the full model of restart-on-visit versus continuous-phase behavior.
