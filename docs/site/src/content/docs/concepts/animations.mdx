...
Animations may also emit a per-frame `rotation` (degrees, clockwise) that the message widget renders through an offscreen rotate around the text block's center — the first consumer is the flair plugin's propeller effect, coming separately.
