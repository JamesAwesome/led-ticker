...
| `two_row`                                                   | no                                     |
| `weather`, `rss_feed`, `coinbase`, `coingecko`, `etherscan` | no                                     |
...
Other widget types (`weather`, `rss_feed`, `two_row`, the crypto widgets) have their own draw paths and ignore `animation` silently — the validator rejects it explicitly to make the no-op visible. For dynamic color on those widgets, use `font_color = "rainbow"` or a gradient.
