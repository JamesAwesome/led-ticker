...
| -------------- | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `magnify`      | `1.3`      | Center scale (both axes). Capped by the panel height: `magnify × font line-height ≤ content_height`, else the widget errors on its first draw and the render breaker disables it. For the default 6×12 font in a 16-row band the ceiling is about `1.33`. |
...
- **Hi-res fonts**: on a scaled sign (bigsign) a hi-res font warps too, subject to the same `magnify × font line-height ≤ content_height` ceiling — a too-tall hi-res font errors on its first draw (the render breaker then disables the widget). On a smallsign (scale 1) a hi-res font renders normally (unwarped) and `led-ticker validate` emits a warning: the pixel-precise warp needs a scaled display.
