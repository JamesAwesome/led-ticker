...
`flair.propeller` and `flair.spinout` are designed to pair: the widget spins in at entry and spins out at exit. See [the full-cycle demo](#full-cycle-propeller--spinout) below for the combined config.
