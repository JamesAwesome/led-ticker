...
<TomlExample
  code={`# woven (default) — the rasta GIF above
...
<DemoGif
  src="/demos-pinned/border-bands-stacked.gif"
  caption="align_rings = true — stacked stripes, rings marching in lockstep"
/>

**Holiday static accent** — a non-moving two-color frame that costs nothing per tick (`speed = 0` rides the fast path):

<TomlExample
  code={`border = {style = "bands", colors = "christmas", speed = 0, band_width = 10}
...
<TomlExample
  code={`border = {style = "bands", colors = "usa", speed = -2}
...
| `gif`                        | yes                        |
| `image`                      | yes                        |
| `feeds.weather`, `feeds.rss` | no — raises at config load |
