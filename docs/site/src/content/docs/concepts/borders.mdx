...
A border paints a decorative outline around the entire panel perimeter. Five styles are available: an animated **rainbow chase** that sweeps a hue gradient per pixel around the edge, a **color_cycle** that pulses the entire border as one animated hue, a **constant color** that frames the content in a steady solid color, **bands** that march discrete solid-color bands around the edge for candy-cane and flag-ribbon looks, and **lightbulbs** that paint discrete bulb sprites for a classic marquee aesthetic — lit bulbs can optionally use a static rainbow coloring. Both `rainbow` and `color_cycle` accept optional `from` / `to` RGB colors to confine the animation to a specific hue arc instead of sweeping the full spectrum. All styles paint at physical panel resolution, so a pixel-ring border is always exactly one LED wide regardless of scale — even on a big / scaled sign (`default_scale > 1`), where most content is drawn in logical pixels and expanded to fill the panel (see [Display](/concepts/display/)). _For developers:_ borders bypass that block-expansion path and write directly to the real underlying canvas (`SetPixel`).
...
| `bands`       | none — inline table only | yes — per-band solid color  | yes       |
...
A constant border is a solid-color ring with no animation. Because it never changes frame to frame (`frame_invariant`, an internal flag), the engine's static-text fast path applies: the border is painted once and the widget sleeps for the hold duration rather than redrawing every tick.
...
`lightbulbs` reads as a **physical object** (a marquee of light fixtures). `rainbow` / `color_cycle` read as a **graphical effect** (a halo or outline). `bands` reads as a **themed ribbon** — choose it when you want a recognizable color sequence (holiday, national flag, sports team) rather than a spectrum sweep. Pick `lightbulbs` for theatrical / vintage / sports-bar / Vegas-pastiche aesthetics; `rainbow` / `color_cycle` for clean / modern / decorative work; `bands` for seasonal or themed displays.
