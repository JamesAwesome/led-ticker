...
With `ttl_seconds = 0` the light stays on until an explicit `state=off` arrives. Set it to a positive value (e.g. `1800`) and the light auto-clears after that many seconds unless a fresh `state=on` refreshes it — a safety net if the sender sleeps mid-meeting and never sends `off`. TTL applies only to the HTTP source — the file source is polled directly and never arms the deadline.
