...
`font_color` accepts five forms: a `[r, g, b]` list (constant), the string shorthands `"rainbow"`, `"color_cycle"`, and `"random"`, or an inline table for a gradient or a ranged color cycle. The first three cover most signs. The same field on `top_color` / `bottom_color` for `two_row` and image widgets behaves identically — pick once per widget, swap providers without changing anything else.
