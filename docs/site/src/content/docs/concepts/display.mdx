...
This is a **brightness** schedule — it never changes what content is shown, only how bright it is. To show or hide a widget or section by time of day (e.g. swap an "OPEN" graphic for "CLOSED"), see [Widget scheduling](/concepts/scheduling/), which uses the same window vocabulary against the sign's content instead.
...
- **Timezone**: the brightness scheduler reads the top-level `[display] timezone` (an IANA name, e.g. `"America/New_York"`, `"Europe/London"`; empty = the Pi's system time). Set it explicitly if you can; freshly imaged Pis default to UTC, which will shift your windows by several hours. `[display.schedule] timezone` is also available as a per-feature override — when set, it wins for brightness only. [Widget visibility schedules](/concepts/scheduling/) always read the top-level `[display] timezone`, so setting only the per-feature override leaves visibility on the system clock. Set the top-level field so both features share one clock.
...
<RelatedPages slugs={["concepts/scheduling", "concepts/fonts", "widgets/two_row"]} />
