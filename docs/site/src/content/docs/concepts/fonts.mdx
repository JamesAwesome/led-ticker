...
Every text-bearing widget — `message`, `countdown`, `two_row`, `feeds.weather`, and the text overlay on `gif`/`image` — picks a font. led-ticker ships two flavors: pre-rasterized BDF bitmap fonts and scalable hi-res TTF/OTF fonts. The right choice depends on your panel scale and viewing distance.
