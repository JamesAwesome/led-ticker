...
Every animated effect in led-ticker — rainbow text, color cycle, typewriter, rainbow chase border — advances based on a frame counter that ticks once per engine iteration. The engine runs at a fixed 50 ms cadence, so one frame is 50 ms. More precisely, **each effect on a widget gets its own independent counter**. A `message` widget with `font_color = "rainbow"`, `border = "rainbow"`, and `animation = "typewriter"` carries three separate counters simultaneously: one for the color sweep, one for the border chase, and one for the typewriter reveal. None of them advance together, and none of them interfere.
