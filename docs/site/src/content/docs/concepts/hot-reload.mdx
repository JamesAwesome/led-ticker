...
If the new config validates but has zero sections (the common typo of `[[sections]]` instead of `[[playlist.section]]`), the panel goes dark for the interlude — there's nothing to display, so it blanks rather than freezing on the last frame — and recovers automatically as soon as a reload lands a config with sections again.
