...
**Logs** — each reload attempt logs at INFO level:

- On success: `config reloaded` (plus a `restart required: <fields>` warning if applicable).
- On failure: `config reload rejected: <validation error>`.
...
# Bare metal
journalctl -u led-ticker -f | grep reload
```
...
The `config` directory is mounted read-only (`./config:/code/config:ro`), which does not prevent the Pi's own editor from updating the host file — the container sees the change through the bind mount. Hot-reload works the same way in Docker as on bare metal.
...
- Check `docker compose logs led-ticker` or `journalctl -u led-ticker` for a `reload rejected` line — that means the new config failed validation and the old one is still running.
