...
- **A section-level `schedule` on a forever-cycling section (`loop_count = 0`) is only checked once, at entry — usually.** The engine re-checks a section-level `schedule` when the section is entered — not on every pass through it. A `loop_count = 0` section cycles forever and never comes back around to be "entered" again unless its widget rotation empties out first. Two shapes are exceptions: a **title-only** section (no widgets) is re-entered every pass, since there's no widget rotation to get stuck inside, so its schedule IS re-checked each time; and a section where **every widget has its own `schedule = {...}`** exits the forever cycle whenever all those windows close, which also re-checks the section-level schedule — just at the widget windows' boundary, not necessarily exactly at the section's own `end`. For everything else — at least one widget without its own schedule — the section keeps showing past its `end` time instead of going dark. Widget-level `schedule = {...}` doesn't have this problem on its own — it's re-checked every pass regardless of `loop_count`. `led-ticker validate` warns when it sees the problematic combination (with a softened message for the all-widgets-scheduled shape); prefer widget-level schedules, or set a finite `loop_count` (>= 1), for a section-level schedule that must actually expire exactly on time.
