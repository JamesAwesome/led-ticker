...
- **A section-level `schedule` on a forever-cycling section (`loop_count = 0`) is only checked once, at entry.** The engine re-checks a section-level `schedule` when the section is entered — not on every pass through it. A `loop_count = 0` section cycles forever and never comes back around to be "entered" again, so its schedule effectively stops mattering after the first check: the section keeps showing past its `end` time instead of going dark. Widget-level `schedule = {...}` doesn't have this problem — it's re-checked every pass regardless of `loop_count`. `led-ticker validate` warns when it sees this combination; prefer widget-level schedules, or set a finite `loop_count` (>= 1), for a section-level schedule that must actually expire.
