...
- **Widget-level `schedule` (and `should_display()`) are re-evaluated every pass through a section — including in a `loop_count = 0` (forever) section.** Both checks live in `_expand_sources`, which runs once per pass regardless of `loop_count`, so a widget's window closing (or a countdown rolling out of range) reaches the panel within a couple of queued items plus whatever widget is currently showing — typically a few seconds; proportionally longer when the showing widgets scroll long text (each queued item is one full widget show), not "whenever the section is next entered." This holds with or without a section-level schedule.
