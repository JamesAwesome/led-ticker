...
- **Widget-level `schedule` (and `should_display()`) are re-evaluated every pass through a section — including in a `loop_count = 0` (forever) section.** Both checks live in `_expand_sources`, which runs once per pass regardless of `loop_count`, so a widget's window closing (or a countdown rolling out of range) reaches the panel within about three in-flight items plus whatever widget is currently showing — i.e. up to ~4 widget shows (roughly 15–30 s at `hold_time = 5`; proportionally longer when the showing widgets scroll long text, since each item is one full widget show), not "whenever the section is next entered." This holds with or without a section-level schedule.
