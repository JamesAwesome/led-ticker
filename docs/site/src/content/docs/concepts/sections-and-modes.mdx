...
Preserved for backward compatibility, but the recommended setup for displaying gifs is `mode = "swap"` with a `gif` widget in the section's `widget` list. Direct use of `mode = "gif"` may be removed in a future release.
