...
One honest scope boundary, narrow:
...
A `typewriter` reveal colors tokens correctly the whole time, including when the message mixes a colored token with inline `:slug:` emoji — each token character takes the source's color as soon as it appears on screen.
