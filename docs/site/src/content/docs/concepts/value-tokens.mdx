...
If a fetch fails, the previous value is kept and the task retries on the next `interval`. A crashed fetch task is supervised and restarted, so a transient network error doesn't freeze the token permanently.

The [weather plugin](/plugins/available/#weather) (`type = "weather.current"`) is a concrete polled source — it fetches current conditions on a configurable interval and formats them into a token string.
