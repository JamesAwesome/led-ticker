...
Colored tokens work in every widget that reads inline tokens: `message`, `two_row` (both rows), and the text overlay on `gif` / `image` widgets. On a `two_row` widget, use `top_color` / `bottom_color` for the row's literal text; a token on that row still takes its source's `color`.

<DemoGif
  src="/demos-pinned/token-color-two-row.gif"
  caption="A `two_row` widget: the `:status:` token renders in its source's green on both rows, while the literal labels keep the row's white."
/>

A `typewriter` reveal colors tokens correctly the whole time, including when the message mixes a colored token with inline `:slug:` emoji — each token character takes the source's color as soon as it appears on screen. A colored token also keeps its color under a [`flair.fisheye`](/concepts/animations/#flairfisheye-flair-pack) lens.
...
### `image` — your own image as an inline emoji

`path` points at a local PNG or GIF under `config/assets/`; the token then
renders the image as an inline sprite wherever emoji do. It's a
declaration, not a live value — the full walkthrough (file placement,
naming, sizing) lives on the [emoji page](/assets/emoji/#your-own-images-as-emoji).
