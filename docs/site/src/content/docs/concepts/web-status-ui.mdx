...
The web UI can trigger that restart for you. When `allow_restart = true` is set in your `[web]` block, a **Restart to apply** button appears on the Status tab and in any "restart required" banner the hot-reload engine shows. Clicking it:

1. Confirms the action (a brief in-button countdown lets you cancel).
2. Posts to `/api/restart` — a token is required if one is configured.
