...
The display process publishes a `status.json` file (schema version 5) containing the state of the running display. The web UI renders this snapshot; advanced users and integrations can also read it directly. Notable fields:
...
**A token is required.** Set `LED_TICKER_WEB_TOKEN` in your `.env` (see [Auth](#auth) above). Without a token, the editor is read-only — Save returns 403. With a token, every other request still requires it, so the token gates both reads and writes when set.
