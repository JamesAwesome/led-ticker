...
**Check for updates.** Click **Check for updates** and the Store resolves the latest version of every installed plugin and of led-ticker-core itself in one pass — it only makes network requests when you click it, never in the background. Plugins with a newer version available get a gated **Upgrade** button next to them. Core is different: it shows a flag only, with no button, because a core update runs through `git pull && make update` on the sign itself rather than pip.
