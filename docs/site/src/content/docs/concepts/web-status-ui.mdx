...
**Deploy note.** The webui container must be able to write the host-bind-mounted `config.toml`. In `compose.yaml` the webui service mounts the config directory `:rw` (the display service still mounts it `:ro`). On bare metal, confirm the user running `led-ticker webui` has write permission to the config directory. If the Pi's config directory is owned by root and the sidecar runs as an unprivileged user, `chown` the directory or add the sidecar user to the owning group.
...
4. Your process supervisor brings the display back. On Docker, `restart: unless-stopped` handles this automatically. On bare metal with systemd, `Restart=always` (or `on-success`) does the same.
...
<Aside type="caution">
  **Bare metal: set up a supervisor before enabling this.** If you enable `allow_restart` without a
  supervisor configured to restart the process, clicking the button leaves the sign dark. On bare
  metal, confirm your systemd unit has `Restart=always` or `Restart=on-success` before flipping this
  on. On Docker, `restart: unless-stopped` is already set in `compose.yaml`.
</Aside>
...
The dashboard header shows the deployed build. It resolves to the first of: the `branch@sha` baked into the Docker image at build time (every build captures it from the source's Git refs — no special flag needed); the `branch@sha` from Git for a non-Docker install running from a checkout (the systemd/venv deploy, or local dev); or the release version (`v2.1.0`) for a `pip install` from PyPI with no source tree. Use this to confirm both containers are running the code you expect.

- **`build unknown`** — none of those were available (no baked stamp, no Git checkout, no installed package metadata). Rare for a real install; the sign still runs correctly, and `unknown` is a label, not an error.
