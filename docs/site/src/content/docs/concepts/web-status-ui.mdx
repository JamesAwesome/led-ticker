...
The dashboard header shows `build <branch>@<sha>` — the Git branch and short SHA baked into the image when it was built. Use this to confirm both containers are running the code you expect.

- **`build unknown`** — the image was not built via `make build-docker` or `make rebuild` (e.g. pulled from a registry without the build args, or started from a dev environment without the stamp step). The sign still runs correctly; `unknown` is a label, not an error.
