...
The dashboard header shows the deployed build. It resolves to the first of: the `branch@sha` baked into the Docker image at build time; the `branch@sha` from Git for a non-Docker install running from a checkout (the systemd/venv deploy, or local dev); or the release version (`v2.1.0`) for a `pip install` from PyPI with no source tree. Use this to confirm both containers are running the code you expect.
