...
- **`⚠ webui build …`** — the webui sidecar is on a different build than the display process. This can happen when only one service is restarted without rebuilding. Run `make rebuild` to bring both containers to the same image at once.
