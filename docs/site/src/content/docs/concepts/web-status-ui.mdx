...
**Changes take effect on the next `docker compose restart`** — the same as editing `config/requirements-plugins.txt` by hand. The Store writes the manifest for you and shows a "pending restart" banner with a copyable restart command once there is a queued change. There is no one-click restart from the browser; you run the command yourself.
