...
The dashboard header shows `build <branch>@<sha>` — the Git branch and short SHA of the deployed code. In Docker it is baked into the image at build time; a non-Docker install running from a checkout (the systemd/venv deploy, or local dev) reads it from Git instead. Use this to confirm both containers are running the code you expect.

- **`build unknown`** — no build identity is available: the image was not built via `make build-docker` / `make rebuild` **and** the process is not running from a Git checkout (e.g. installed from PyPI with no source tree). The sign still runs correctly; `unknown` is a label, not an error.
