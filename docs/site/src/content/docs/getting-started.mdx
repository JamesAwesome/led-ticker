...
:::tip[No hardware?]
You can preview any config as a GIF on your laptop — build a config and
watch it run before buying a single panel. After `make dev`, run
`make render-demo CONFIG=config/config.example.toml OUT=preview.gif` to
render at native panel resolution; see [render-demo](/tools/render-demo/)
for the full software-first workflow.
:::

## Install

Requires Python 3.14+ and [uv](https://docs.astral.sh/uv/) (the package
manager these commands use). Install uv with `pip install uv` or
`brew install uv`.
...
[Validation rules](/pitfalls/) page for the full rule list.
