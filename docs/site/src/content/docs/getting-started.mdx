...
Get a live preview of led-ticker running on your laptop in one command:
...
Once your Pi is wired up with a HUB75 panel and Adafruit HAT or Bonnet:
...
example, seeds a `.env` stub, and brings the stack up.
...
**3. Bring up the stack** (after editing `config.toml`):
...
or RGB-order problems. Then use the
...
The [`render-demo`](/tools/render-demo/) tool runs the real ticker engine
against a software stub and writes each config to a GIF, so you can
iterate on your config before buying a single panel:
