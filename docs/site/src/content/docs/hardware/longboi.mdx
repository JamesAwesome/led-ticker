...
| Setting                 | Value       | Why                                                                                           |
| ----------------------- | ----------- | --------------------------------------------------------------------------------------------- |
| `gpio_slowdown`         | `5`         | Pi 5 with `rp1_rio=1` needs higher slowdown than Pi 4. Raise to 4–5 if flicker persists.      |
| `rp1_rio`               | `1`         | RIO (Registered IO) mode on the Pi 5 RP1 — faster refresh, slightly more CPU than PIO (`0`).  |
| `row_address_type`      | `0`         | Required when `rp1_rio = 1`. Do not set to `1` — it breaks display output with RIO mode.      |
