...
Other widget types have their own draw paths and ignore `animation` silently — the validator rejects it explicitly to make the no-op visible. For dynamic color on `weather`, `rss_feed`, the crypto widgets, or `two_row`, use `font_color = "rainbow"` or a gradient instead.
