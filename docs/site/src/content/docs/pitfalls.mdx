...
### Rule 36 — `loops = 0` in `mode = "gif"` silently plays 1 loop

`loops = 0` means "play through the section's `hold_time`" — but that plumbing only exists on the `mode = "swap"` path. The legacy `mode = "gif"` calls the gif widget's `play()` directly without threading `hold_time`, so the "play through hold_time" semantics don't reach there. A `loops = 0` widget in a `mode = "gif"` section silently falls back to exactly one loop. Switch the section to `mode = "swap"` (the recommended setup; see rule 33) to get the intended duration-driven playback, or set `loops` to an explicit positive integer for a fixed loop count.
