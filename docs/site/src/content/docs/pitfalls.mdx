...
Other widget types have their own draw paths and ignore `animation` silently — the validator rejects it explicitly to make the no-op visible. For dynamic color on `feeds.weather`, `feeds.rss`, the crypto widgets, or `two_row`, use `font_color = "rainbow"` or a gradient instead.
...
### Rule 33 — prefer `mode = "swap"` with a `gif` widget over `mode = "gif"`

Prefer `mode = "swap"` with a `gif` widget in the section's `widget` list — it gives you the full section feature set (transitions, `hold_time`, `bg_color`, multi-widget sections). `mode = "gif"` is a narrower direct-play form that doesn't reach those knobs. To switch, change `mode = "gif"` to `mode = "swap"` and remove any mode-specific knobs that `swap` doesn't need.
