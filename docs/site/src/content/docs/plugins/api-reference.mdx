...
| `draw_with_emoji(canvas, font, cursor_pos, y, color, text, ...)`           | Full rich-text renderer: inline `:emoji:` + per-char `ColorProvider` colors (`draw_text` is the simpler `Color`-only form)                                                                                                    |
