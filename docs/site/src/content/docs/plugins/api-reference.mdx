...
| `hires_text_width(text, size, font=..., threshold=...)`                    | Physical advance width of hi-res text, measured with the renderer's own glyph resolution (collision math can't drift from the paint)                                                                                          |
| `fit_text_size(text, sizes, max_width, font=..., threshold=...)`           | Largest size from a caller-owned ladder at which text fits a px budget (shrink-to-fit guard for positioned layouts)                                                                                                           |
...
An animation can instead return a **`LensSpec`** on `AnimationFrame.lens` (default `None`) to pass the scrolling text through a stationary fisheye lens — the engine owns all of the warp math, so a plugin ships only the declarative spec (`magnify`, `edge_squeeze`, `profile`) and sets `emits_lens = True` as a class attribute (the marker for validate rule 64, the lens twin of rule 63's hi-res-font-at-scale-1 warning). The lens works on `message`'s single string draw and on the `gif`/`image` single-row text overlay (`_BaseImageWidget`) via the same shared `LensTextRenderer` — `emits_lens` is duck-typed, so a plugin's lens animation needs no per-widget-type wiring to reach either surface. On `gif`/`image`, `emits_lens` also relaxes the scroll-mode refusal that applies to other animations (a lens is compatible with `text_align = "scroll"` / `"scroll_over"` — the scroll IS the effect) but keeps three single-row-only refusals (`bottom_text` set, non-center `text_valign`, `text_wrap`); a static-dict twin of these three (validate rule 65) surfaces them at config-load rather than at widget construction.
...
- **`RESOLVES_OWN_FONT`** — a widget class may set `RESOLVES_OWN_FONT = True` to receive its configured `font` as the raw TOML name string instead of a resolved `Font`/`HiresFont` object. Normally the config loader eagerly resolves `font` (and raises if a hi-res name is given without `font_size`); with the marker set, that resolution and the `font_size` requirement are both skipped, so a widget that auto-sizes its own font per use (e.g. sizing pixel-art glyphs to fit a dynamic layout) can call `resolve_font` itself. Default `False` (unset behaves the same as `False`) — every other widget's font handling is unaffected.
- **`_LIST_FIELD_HINTS`** — a widget class may set `_LIST_FIELD_HINTS: dict[str, tuple[str, str, str]]` to supply the human-readable `--list-fields` hint for its own fields. Each value is `(display_type, description, default_display)` — the enum/type string, the prose, and the shown default. These are resolved **ahead of** core's built-in hint table, so they're the correct home for a field whose meaning is plugin-specific. In particular a `layout` field: every layout-bearing plugin has its own enum, so core carries **no** name-keyed hint for it (a shared hint would be wrong for all plugins but one) — declare your own here. A field with no per-widget hint falls back to its plain annotation and default. Example:
  ...
  class Forecast:
      ...
      _LIST_FIELD_HINTS = {
          "layout": (
              '"auto" | "strip" | "big" | "long"',
              "forecast render mode",
              '"auto"',
          ),
      }
  ```

  A malformed entry (not a 3-tuple or `FieldHint`) fails `--list-fields` loudly, naming the widget and field.
