...
An animation that emits non-zero rotations must also set `emits_rotation = True` as a class attribute. This opt-in marker tells the validator to run rule 63: a widget that pairs a rotation-emitting animation with a hi-res font raises a configuration warning, because hi-res text renders at native-pixel resolution and the offscreen rotate path operates at logical resolution — the two interact badly. For BDF fonts the rotate path is fully supported.
