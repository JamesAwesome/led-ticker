...
| `FrameAwareBase`    | Mixin to inherit (with `@attrs.define`) so per-tick `advance_frame()` drives animated `font_color` / `border` |
...
| `ScaledCanvas`   | The wrapper widgets receive when `default_scale > 1`; `isinstance(canvas, ScaledCanvas)` gates hi-res paths   |
...
| `draw_with_emoji(canvas, font, x, y, color, text)` | Lower-level text+emoji draw; returns the advance width (what `draw_text` wraps) |
...
| `normalize_bg(value)`                              | Coerce a bg value (`Color`/`[r,g,b]`/`None`) to an `(r, g, b)` tuple or `None`  |
