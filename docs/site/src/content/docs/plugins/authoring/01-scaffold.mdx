...
<Aside type="note" title="Before you start">
  You'll work inside a led-ticker checkout. If you haven't already, clone it and run `make dev` —
  that installs the project venv and puts the `led-ticker` command on your `PATH`. Your plugins live
  in `config/plugins/`, next to `config/config.toml`. To run `led-ticker plugins` below: with the
  venv active, run it directly; under Docker, run `docker compose exec led-ticker led-ticker
  plugins`.
</Aside>
...
Now make led-ticker discover it, then verify it loads.
