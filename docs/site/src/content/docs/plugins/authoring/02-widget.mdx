...
<Aside type="tip" title="Color fields are coerced for you">
  ...
  the same way. `font_color` — the standard text-color key — is different: it coerces to a
  _color provider_, an object with a `color_for(...)` method that can yield a constant color, a
  rainbow, a gradient, and more. So a widget that declares a `font_color` field reads
  `self.font_color.color_for(...)` rather than treating it as a plain `Color`. See
  [Color providers](/concepts/color-providers/) for the full list and how config maps to each.
</Aside>
