...
<Aside type="note" title="cursor_pos and font_color">
  ...
  `draw_text` as the x argument (drawing at a hardcoded `0` breaks scrolling). `font_color` is an
  optional per-frame color the engine may inject (e.g. a color provider); honor it when your widget
  should follow a theme. This widget uses its own `color` field instead.
</Aside>
