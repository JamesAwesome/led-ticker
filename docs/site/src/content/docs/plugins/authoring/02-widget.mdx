...
<Aside type="tip" title="Color fields are coerced for you">
  ...
  the same way. The `font_color`, `top_color`, and `bottom_color` keys are different: they coerce to
  a _color provider_ — an object with a `color_for(...)` method that can yield a constant color, a
  rainbow, a gradient, and more — so a widget reads `self.font_color.color_for(...)` rather than
  treating it as a plain `Color`. See [Color providers](/concepts/color-providers/) for the full
  list and how config maps to each.
</Aside>
