...
Add the package to your led-ticker checkout's `config/requirements-plugins.txt` and restart. A startup reconcile makes the installed plugins match the file — no image rebuild. The install flow (the constraint-based install, the gitignored live file) is covered on the [Plugins overview](/plugins/) — in short:
