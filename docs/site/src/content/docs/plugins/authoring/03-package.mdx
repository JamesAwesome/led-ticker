...
<Aside type="note" title="led-ticker isn't on PyPI (yet)">
  Plugins are installed `--no-deps`-free but **constrained to core's versions** — a plugin can bring
  its own deps but can't move led-ticker's. Because led-ticker is already in the image, your
...
<Aside type="caution" title="Tests may use internals">
  ...
  `tests/conftest.py`. In a standalone plugin repo, copy that small `canvas` fixture into your own
  `conftest.py` (the `led-ticker-pool` repo does exactly this).
</Aside>

Testing in a standalone plugin repo? Add the `canvas` fixture to your own `conftest.py`:
...
Once the package is installed, render any config that uses your widget straight to a GIF:

```bash
make render-demo CONFIG=path/to/a-config-using-your-widget.toml OUT=preview.gif
```

`render-demo` runs the real engine, which discovers your installed plugin through its entry point. (A local `config/plugins/` drop-in isn't picked up by `render-demo` — install the package first.)
