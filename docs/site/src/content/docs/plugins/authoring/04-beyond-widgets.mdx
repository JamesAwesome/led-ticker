...
`spawn_tracked` keeps the background task alive; `on_startup` runs it once before the display loop. See [`acme`](https://github.com/JamesAwesome/led-ticker/blob/main/examples/plugins/acme/__init__.py) for a runnable version and the [`pool`](https://github.com/JamesAwesome/led-ticker-plugins/tree/main/plugins/pool) package for a production one.
