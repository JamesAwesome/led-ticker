...
def register(api):
    ...
    # on_startup runs once before the display loop; spawn_tracked launches the
    # poller as a background task and keeps a reference so it isn't garbage-collected.
