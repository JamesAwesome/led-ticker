...
<Aside type="tip">
  ...
  `char_index` / `total_chars` in `color_for` — e.g. offset the hue by `char_index`. And set
  `restart_on_visit = False` to keep the animation's phase continuous across a section's
  `loop_count`.
</Aside>
