...
3. Install your plugin so led-ticker can find it. `make render-demo` only loads **installed** plugins (via their entry point), not a local plugin directory — so package-install yours first (see [Package & install](/plugins/authoring/03-package/)):
...
<Aside type="caution">
  `make render-demo` discovers only **installed** plugins. A plugin you've merely dropped into
  `config/plugins/` loads when you run `led-ticker` on the sign, but won't show up in a
  `render-demo` preview until you `pip install -e .` it.
</Aside>
...
- **Hi-res** — a larger sprite in **physical** pixel coordinates. Used on **scaled (big) signs** (`default_scale > 1`) and by direct draws ([`draw_emoji_at`](/plugins/api-reference/#exported-names)).
