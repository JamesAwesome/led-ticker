...
`render_hires_frame` decodes the sprite to the panel's native height, drives the traversal from `t`, and calls `snap_reset` at `SNAP_THRESHOLD` automatically. In `frame_at`, gate on `isinstance(canvas, ScaledCanvas)` (only `True` on the bigsign) and fall back to your own lo-res drawing on the small sign or in test stubs:
    ...
    class Zoom:
        def frame_at(self, t, canvas, outgoing, incoming, **kwargs):
            if t >= 1.0:
                incoming.draw(canvas, cursor_pos=0)
                return canvas
            if isinstance(canvas, ScaledCanvas):   # bigsign -> hi-res sprite
