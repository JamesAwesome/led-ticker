---
title: Plugins
description: Extend led-ticker with plugins — widgets, transitions, emojis, fonts, and lifecycle hooks — installed via a requirements file and auto-registered at startup.
---
...
The Store doesn't show Upgrade for every plugin up front — click **Check for
updates** first. It resolves the latest version for every declared, active
plugin (a pip-installed-but-pending-restart plugin is skipped; a package
shared by several plugin lines is only resolved once) and labels
each line **Upgrade → `<version>`** only where a newer or newly-pinnable
version is actually available; a plugin already on the newest version shows
**Up to date** instead. Results are per-session — re-run the check after you
install, remove, or hand-edit `requirements-plugins.txt`, since the last
check no longer reflects the current manifest. Checking needs the same token
as Install/Remove/Upgrade.
