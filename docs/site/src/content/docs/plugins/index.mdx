...
- **`Unknown widget type: 'pool.monitor'`** at validate/startup → the plugin isn't installed. Confirm you added it to `config/requirements-plugins.txt` and rebuilt the image (`docker compose up -d --build`). `led-ticker plugins` lists what's currently registered.
