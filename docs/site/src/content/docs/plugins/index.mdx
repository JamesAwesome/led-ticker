...
Adding or removing a plugin no longer requires a Docker image rebuild. Edit `config/requirements-plugins.txt`, then `docker compose restart` — the change applies on the next start in seconds.
:::
...
This is also the fix if a plugin volume becomes corrupt or out of sync.
...
- **Build a custom image** with the plugin baked in (`docker compose up -d --build` on a machine with network access — this is still supported and appropriate for air-gapped deployments).
