...
led-ticker plugin install pool    # add it to requirements-plugins.txt + install
```

`plugin install` resolves the name to a pip source (git or PyPI), appends it to `config/requirements-plugins.txt`, and installs it under core's dependency constraints. A name that isn't in the catalog is treated as a raw pip spec (a `git+https://…` URL or a PyPI requirement), just like `pip install`. See the [CLI reference](/reference/cli/#led-ticker-plugin) for all flags (`--source`, `--unpinned`, `--save-only`, `--dry-run`).
