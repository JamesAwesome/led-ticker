...
- **Build a derivative image** that bakes the plugin in: write a small Dockerfile that starts `FROM led-ticker` and adds a `RUN pip install …` layer with your plugins, build it on a machine with network access, then deploy that image to the air-gapped sign.
