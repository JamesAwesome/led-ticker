...
(`config/requirements-plugins.txt`) only — the **Docker** path, where the image
build does the install — and **`install` / `uninstall`** also run pip, for
bare-metal/dev. (The old `led-ticker plugins` is a deprecated alias of
`plugin status`.)

```bash
# Docker: edit the manifest, then rebuild
led-ticker plugin add pool          # + line in requirements-plugins.txt (no pip)
led-ticker plugin remove pool       # − line (no pip)
docker compose up -d --build        # the build installs from the manifest
...
`config/requirements-plugins.txt` (pinned to the catalog ref by default; a typo
close to a catalog name gets a "did you mean …?"). `remove` drops the matching
line (idempotent — "not in …" if absent). Neither runs pip; both remind you to
rebuild. This is the Docker workflow: the container is read-only at runtime, so
the image build is what installs from the manifest.
...
| `--unpinned`      | off (pinned) | (`add`/`install`) Write `@main` / a bare PyPI name instead of the catalog's recommended pin.         |
...
`requirements-plugins.txt`, so it installs on the next build) and/or
