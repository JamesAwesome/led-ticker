...
and rebuild — it's the same manifest.
...
matching line (idempotent — "not in …" if absent). Neither runs pip; both remind
you to rebuild. This is the Docker workflow: the container is read-only at
runtime, so the image build is what installs from the manifest.
