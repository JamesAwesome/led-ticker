...
"run the display" mode and `validate` and `plugins` subcommands. Day-to-day
...
Lists the plugins discovered for a config — both the ones that loaded
and any that failed — without opening the panel. For each loaded plugin
it prints the namespace, its source, and the surfaces it contributes
(widgets, transitions, fonts, or hooks only); failed plugins are listed
with their load error. Handy for confirming a freshly installed plugin
is being picked up before you deploy.

```bash
led-ticker plugins                          # uses the top-level --config (config.toml)
led-ticker plugins --config config/config.toml
```

| Flag              | Default       | Notes                                                                |
| ----------------- | ------------- | -------------------------------------------------------------------- |
| `--config` / `-c` | `config.toml` | Path to the config whose plugins to resolve (defaults to top-level). |

Exits `0` after printing the summary, or `2` if the config file can't be
read or parsed.
