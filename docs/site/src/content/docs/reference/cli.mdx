...
| `--backend`       | _(config)_    | Override the display backend (e.g. `headless`). Takes precedence over `[display] backend` in the config. Unknown names fail with a clear registry error. Primarily used by the `make try` Docker flow to run headless without modifying your deploy config. |
...
dev / source-checkout installs. **`upgrade`** rewrites an already-declared
line to the newest version — manifest only, like `add`/`remove`, no pip; the
next restart's reconcile installs it. (The old `led-ticker plugins` is a
...
# Move a declared line to the newest version (manifest only — no pip either way)
led-ticker plugin upgrade pool      # rewrite pool's line to the latest version
led-ticker plugin upgrade --all     # same, for every declared plugin
docker compose restart              # installs the new version on the next start
...
All five management verbs accept a **catalog name or a raw pip spec** (a
...
`--unpinned`; all take `--dry-run`. `upgrade` additionally takes `--all` (upgrade
every declared line, mutually exclusive with a target name) — it never runs pip,
even under `install`'s manifest-plus-pip pairing, since it only rewrites an
already-declared line rather than adding or removing one.
...
### `plugin upgrade` (manifest only — resolves the newest version)

Rewrites an already-declared manifest line to the newest version and never
runs pip — the new version installs on the next restart, the same way
`add`/`remove` do. Give a target (catalog name or pip spec, matched the same
way `remove` matches lines) to upgrade one plugin, or `--all` to upgrade
every declared line in one pass; the two are mutually exclusive. "Newest"
comes from PyPI for a PyPI line, or from git tags matching `<name>-vX.Y.Z`
for a git line (falling back to the tracked branch's tip commit if no tag
matches) — see [Upgrading plugins](/plugins/#upgrading-plugins) for the full
resolution order. A `@main` line becomes a pinned line on its first upgrade;
the replaced line is kept as a `# upgraded <date>, was <old>` comment. A
target that isn't declared yet is an error — `upgrade` only moves an
existing pin forward, it doesn't add one (use `plugin add`/`install` first).

```bash
led-ticker plugin upgrade pool          # rewrite pool's line to the latest version
led-ticker plugin upgrade --all         # upgrade every declared plugin
led-ticker plugin upgrade pool --dry-run   # preview without writing
```

| Flag              | Default | Notes                                                                                                   |
| ----------------- | ------- | ------------------------------------------------------------------------------------------------------- |
| `target`          | —       | Catalog name or pip spec to upgrade. Required unless `--all` is given; mutually exclusive with `--all`. |
| `--all`           | off     | Upgrade every line declared in the manifest instead of one target.                                      |
| `--dry-run`       | off     | Print what would change (old line → new line) without writing anything.                                 |
| `--config` / `-c` | (none)  | Non-standard layout only: use `requirements-plugins.txt` next to this config instead of `./config/`.    |

Exits `0` when every requested line is already up to date or was upgraded
successfully, `1` if the resolver couldn't determine a newest version for a
requested line (PyPI/git unreachable, no matching tags and no resolvable
branch tip), and `2` on a usage/manifest error (target not declared, or the
manifest couldn't be written). Under `--all`, one line's failure doesn't
block the others — every line is attempted and the command exits with the
worst code seen (a mix of resolver failures and write failures still exits
non-zero). A resolver failure never touches the manifest for that line —
only a successful resolution is written.
...
The everyday ladder for running a sign, in roughly the order you use it:
...
The `led-ticker` CLI accepts `--config` / `-c`, `--backend`, and the `validate`
...
[Reference: Config options](/reference/config-options/) for the `[display]` field list.
