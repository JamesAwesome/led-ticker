...
| `allow_restart` | bool   | `false`                         | Expose a "Restart display" button in the web UI. When `true`, clicking the button drops a marker file that the display process detects on its next loop pass and calls `sys.exit(0)`, so the supervisor (Docker or systemd) restarts it. Set to `false` (default) to disable the control entirely. |
