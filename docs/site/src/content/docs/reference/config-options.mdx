...
| `separator_color`     | list / string  | `null`     | Color of the `scroll` transition's separator dot. Same shapes as a widget `font_color` (`[r,g,b]`, `"rainbow"`, `"color_cycle"`, `{style=…}`). Only honored by `type = "scroll"`; rejected on other transition types. `null` defaults to white. |
