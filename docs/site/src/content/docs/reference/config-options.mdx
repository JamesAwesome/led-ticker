...
Applies to all numeric fields across `[display]`, `[[playlist.section]]`, and per-widget configs — including `rows`, `cols`, `chain_length`, `parallel`, `gpio_slowdown`, `pwm_bits`, `pwm_lsb_nanoseconds`, `pwm_dither_bits`, `rp1_rio`, `font_size`, `font_threshold`, `hold_time`, `brightness`, `content_height`, `scale`, `loop_count`, `scroll_step_ms`, `start_hold`, `separator_font_size`, and others.
...
| `gpio_slowdown`            | int    | `1`              | GPIO timing slowdown. Smallsign uses `2`; bigsign uses `3` (paired with `rp1_rio = 1`). Raise on flicker.                                                                                                                             |
...
| Field         | Type   | Default                         | Notes                                                                                                                                                 |
| ------------- | ------ | ------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| `host`        | string | `"0.0.0.0"`                     | Interface the sidecar binds. `"0.0.0.0"` listens on all interfaces (all LAN addresses); `"127.0.0.1"` limits to localhost only.                       |
| `port`        | int    | `8080`                          | HTTP port the sidecar serves. Binds directly on the Pi (the compose service uses host networking; bare metal binds it natively).                      |
| `token`       | string | `""`                            | Optional shared secret. Empty = open (trusted-LAN posture). Non-empty requires an `X-Web-Token` header or `?token=` query parameter on every request. |
