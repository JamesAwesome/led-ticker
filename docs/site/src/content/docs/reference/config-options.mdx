...
Applies to `font_size`, `font_threshold`, `hold_time`, `brightness`, `content_height`, `scale`, `loop_count`, and every other numeric field on `[display]`, `[[playlist.section]]`, and per-widget configs.
...
| `transition`           | string / table               | inherits global `default`         | Inter-widget transition for `swap` mode. ALSO becomes the entry transition for this section when explicitly set (see Tips). Accepts a string (`"wipe_left"`) or an inline table for multiple knobs in one field — see "Inline-table transition form" below.                                                                                                                                                                                                                                |
