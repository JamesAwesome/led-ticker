...
Applies to all numeric fields across `[display]`, `[[playlist.section]]`, and per-widget configs — including `rows`, `cols`, `chain`, `parallel`, `slowdown_gpio`, `pwm_bits`, `pwm_lsb_nanoseconds`, `rp1_rio`, `font_size`, `font_threshold`, `hold_time`, `brightness`, `content_height`, `scale`, `loop_count`, `scroll_step_ms`, `start_hold`, `separator_font_size`, and others.
