...
| `token`         | string      | `""`          | Shared secret for the HTTP source. Empty = open (trusted LAN only). Prefer `LED_TICKER_BUSY_TOKEN` in `.env`; this field is a fallback and logs a warning when non-empty. |
...
| `token`       | string | `""`                            | Optional shared secret. Empty = open (trusted-LAN posture). Non-empty requires an `X-Web-Token` header or `?token=` query parameter on every request. Prefer `LED_TICKER_WEB_TOKEN` in `.env`; this field is a fallback and logs a warning when non-empty. |
