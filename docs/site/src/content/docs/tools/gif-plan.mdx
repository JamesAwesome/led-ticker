...
| `2`  | The `# render-duration:` header is shorter than the estimate (expected, and ignorable, when `loops` is intentionally large). |
...
- For looping or large-`loops` demos (the `loops = 999` "keep animating" idiom) the recommended `--duration` is the full multi-minute playthrough, not the capture window — such demos deliberately pin a short `# render-duration:` header; render with the header, treat the recommendation as an upper bound.
