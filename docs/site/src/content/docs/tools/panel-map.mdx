...
Only multi-row layouts (e.g. the bigsign's 2×4 serpentine) need a
`pixel_mapper_config`. If your panels are a single horizontal chain — like
the smallsign — skip this page; [`panel-test`](/tools/panel-test/) is all
you need.
:::
...
- The led-ticker Docker image built once: `make build` (the `-docker`
...
Or just open `layout.txt` in any editor (`nano layout.txt` on the Pi) and type
the grid.
