...
| `mode = "gif"` → prefer `mode = "swap"` (rule 33)                | warning  | use `mode = "swap"` with a `gif` widget                             |
...
The ticker will start, scroll, and swap when only warnings are present, but the warning usually points at a rendering quirk you don't actually want — a `two_row` widget cramped into a 64px logical canvas, a `transition_duration` that looks like milliseconds when the engine wants seconds, or a `mode = "gif"` section that would benefit from the full `swap`-mode feature set. Treat them as a checklist before you call a config done.
