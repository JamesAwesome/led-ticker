...
| `start_hold` on `slideshow` / `gif` section or negative value    | error    | remove `start_hold` or set it to ≥ 0                                |
...
| `mode = "gif"` → prefer `mode = "slideshow"` (rule 33)           | warning  | use `mode = "slideshow"` with a `gif` widget                        |
| `default = "..."` written inside a section block (rule 35)       | warning  | rename to `transition = "..."`                                      |
| `play_count = 0` inside a `mode = "gif"` section (rule 36)       | warning  | switch to `mode = "slideshow"` or set positive count                |
