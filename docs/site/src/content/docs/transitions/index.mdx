...
**The `transition` shortcut:** writing `transition = "pokeball"` on a section sets pokeball for _both_ the entry and inter-widget swaps. This is usually what you want for single-widget sections. For multi-widget sections use `entry_transition` + `widget_transition` to control the two slots independently.
...
- `show_pikachu` / `show_pokeball`: toggle sprite elements on pokeball-family transitions
