...
| [Sprite](/transitions/sprite/)   | Pixel-art character (Pikachu, Pac-Man, Nyan Cat, …) crosses the panel | Fun / themed                      |
...
| Playful     | `arcade.pokeball_alternating` or `arcade.nyancat_alternating` (plugin) | `arcade.pokeball_alternating` |
...
**The `transition` shortcut:** writing `transition = "arcade.pokeball"` on a section sets that transition for _both_ the entry and inter-widget swaps. This is usually what you want for single-widget sections. For multi-widget sections use `entry_transition` + `widget_transition` to control the two slots independently.
...
- `show_pikachu` / `show_pokeball`: toggle sprite elements on the `arcade.pokeball` family ([led-ticker-arcade](https://github.com/JamesAwesome/led-ticker-arcade) plugin)
