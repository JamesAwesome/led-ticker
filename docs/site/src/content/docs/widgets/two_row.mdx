...
Each row gets a slice of `content_height` — 50/50 by default, or per `top_row_height` when set. If a row's font has a logical line-height taller than its band, glyphs would clip into the other row's space, so the validator rejects it at config-load. The fixes: pick a smaller `font_size`, raise `content_height` so each band is taller, adjust `top_row_height` to give the offending row more space, or fall back to a BDF alias (`5x8`, `6x12`) whose fixed cell heights tend to fit small bands more reliably than hi-res fonts sized for visual weight. For `two_row` at `scale = 2`, the canonical pairing is BDF `5x8` on `content_height = 16`; Inter at sizes 14+ usually needs `content_height = 24` or larger.
