...
<DemoGif
  ...
  caption="held magenta `NOW PLAYING` on top, cyan song title flies through offscreen-to-offscreen on the bottom. `hold_time = 10.0` drives the pass count via max-of math — no `bottom_text_loops` set, so the engine fills the section duration with as many passes as fit"
/>
