...
- Mutually exclusive with `bottom_text_wrap = true` — pick one. Scroll-through is one pass per visit; wrap is a seamless tiled marquee.
...
- **Section `hold_time` is IGNORED on `two_row`.** The widget exits as soon as the scroll pass completes, regardless of `hold_time`. Pass duration is `(canvas.width + bottom_width) × scroll_step_ms` milliseconds. If you need post-scroll dead time before the section transitions, follow this widget with another widget in the same section.
