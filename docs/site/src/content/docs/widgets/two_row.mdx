...
The hi-res sprite is 32 real pixels tall. At `scale = 2` it's 16 logical pixels, which exactly matches `top_row_height = 16` — but the default vertical placement centers an 8-row low-res sprite (the formula puts the sprite at row 4 of the band), which leaves the 16-row hi-res sprite bleeding 4 rows into the bottom row. Set `top_emoji_y_offset = -4` to pull the sprite up to the band's top edge. Smaller bands fall back to the 8×8 lo-res sprite automatically.
