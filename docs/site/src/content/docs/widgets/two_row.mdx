...
- **Section `hold_time` is IGNORED on `two_row`.** The widget exits as soon as `bottom_text_loops` passes complete (or one pass if loops = 0), regardless of `hold_time`. Pass duration is `(canvas.width + bottom_width) × scroll_step_ms` milliseconds; total visit = `loops × pass_duration`.
- **Important:** the same field on [`gif`](/widgets/gif/) and [`image`](/widgets/image/) widgets behaves DIFFERENTLY: `hold_seconds` is honored as a _floor_ there (the marquee repeats to fill the hold time). If you mix the two widget types in one section, the section's effective length depends on which widget is up. Lock the asymmetry in your mental model or pick one widget type.
...
- **With `bottom_text_scroll = "scroll_through"`**: exact number of offscreen-to-offscreen passes. `hold_time` is ignored entirely on this path — the widget always runs exactly `bottom_text_loops` passes (or 1 if loops = 0).
