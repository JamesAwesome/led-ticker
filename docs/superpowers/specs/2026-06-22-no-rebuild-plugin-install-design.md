...
- **Third-party namespace round-trip mismatch:** a third-party plugin whose pip dist name differs from its registered entry-point namespace can cause reinstall/attempted-uninstall churn on every boot (declared manifest key ≠ installed namespace → reconcile sees a perpetual diff). First-party catalog plugins are unaffected (catalog maps dist name → namespace explicitly). Full third-party namespace handling is a Spec-2 concern.
