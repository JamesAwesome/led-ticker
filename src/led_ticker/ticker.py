...
async def _play_widget(
    ...
    """Shim: delegate to ``Ticker._play_widget`` (frame → self.frame)."""
    t = Ticker(monitors=[], frame=frame)
    return await t._play_widget(canvas, widget, section_hold_time=section_hold_time)
...
async def _scroll_between(
    ...
    """Shim: delegate to ``Ticker._scroll_between``."""
    t = Ticker(monitors=[], frame=frame, scroll_speed=scroll_speed)
    return await t._scroll_between(canvas, outgoing, incoming, outgoing_scroll_pos)
...
async def _swap_and_scroll(
    ...
    """Shim: delegate to ``Ticker._swap_and_scroll``."""
    t = Ticker(monitors=[], frame=frame, scroll_speed=scroll_speed)
    return await t._swap_and_scroll(
        canvas,
        ticker_obj,
        hold_time=hold_time,
        skip_initial_draw=skip_initial_draw,
...
async def _show_one(
    ...
    """Shim: delegate to ``Ticker._show_one``."""
    t = Ticker(monitors=[], frame=frame, scroll_speed=scroll_speed)
    return await t._show_one(
        canvas,
        widget,
        hold_time=hold_time,
        skip_initial_draw=skip_initial_draw,
...
async def _run_swap(
    ...
    """Shim: delegate to ``Ticker._run_swap``."""
    t = Ticker(
        monitors=[],
        frame=frame,
        notif_queue=notif_queue,
