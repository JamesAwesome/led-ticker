...
def test_pillarbox_image_align_left_anchors_at_x_zero(tmp_path):
    """image_align='left' positions a 64×64 pillarboxed frame at x=0..63;
    the right side (cols 64..255) is the black pillar."""
    path = tmp_path / "sq.gif"
    path.write_bytes(_make_gif([(255, 255, 255)], size=(32, 32)).getvalue())

    [(pixels, _)] = decode_gif(
        path, panel_w=256, panel_h=64, fit="pillarbox", image_align="left"
    )
...
def test_pillarbox_image_align_right_anchors_at_panel_edge(tmp_path):
    """image_align='right' positions the 64×64 frame at cols 192..255 with
    the black pillar covering cols 0..191."""
    path = tmp_path / "sq.gif"
    path.write_bytes(_make_gif([(255, 255, 255)], size=(32, 32)).getvalue())

    [(pixels, _)] = decode_gif(
        path, panel_w=256, panel_h=64, fit="pillarbox", image_align="right"
    )
...
def test_unknown_image_align_raises(tmp_path):
    path = tmp_path / "x.gif"
    path.write_bytes(_make_gif([(0, 0, 0)]).getvalue())

    with pytest.raises(ValueError, match="image_align"):
        decode_gif(path, panel_w=256, panel_h=64, fit="pillarbox", image_align="weird")


@pytest.mark.parametrize("fit", ["stretch", "letterbox", "crop"])
def test_image_align_is_noop_on_full_width_fits(tmp_path, fit):
    """`image_align` only matters for pillarbox where the scaled image is
    narrower than the panel. The other three fits fill the panel width
    — image_align must produce identical output regardless of its value.
    Pins the no-op behavior so a regression wiring image_align into
    ...
    [(left_pixels, _)] = decode_gif(
        path, panel_w=256, panel_h=64, fit=fit, image_align="left"
    )
    [(center_pixels, _)] = decode_gif(
        path, panel_w=256, panel_h=64, fit=fit, image_align="center"
    )
    [(right_pixels, _)] = decode_gif(
        path, panel_w=256, panel_h=64, fit=fit, image_align="right"
    )
